...
- **Session ID remap lifecycle** — New API sessions start with a temp UUID; the SDK returns the real CLI session ID on completion. `runSession()` calls `watcher.remap(oldId, newId)` + `transitionToPoll()`, then pushes `session_redirected`. The client sets `currentSessionId = newId` and `connect(newId)` short-circuits (returns early, keeps the existing socket alive via `_redirectingTo`). The socket's `doConnect()` closure retains the old `sessionId` — any `sessionId`-based comparisons inside that closure are stale after remap.
